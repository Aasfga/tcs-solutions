...
drop table if exists Ceny_Dostaw cascade;
create table Ceny_Dostaw (
	waga_zamowienia numrange constraint pk_cen_dos primary key,
	cena_dostawy numeric(6,2) not null

	check (cena_dostawy >= 0)
);
...
insert into Ceny_Dostaw (waga_zamowienia, cena_dostawy) values
	(numrange(0, 100, '[]'), 0), 
	(numrange(100, 500, '(]'), 7),
	(numrange(500, 1000, '(]'), 12),
	(numrange(1000, 5000, '(]'), 25),
	(numrange(5000, 15000, '(]'), 50),
	(numrange(15000, NULL, '()'), 80);
